...
""",
    ...
    def test_capture_replicate_deploy_config_can_disable_replication_for_deploy_groups(self) -> None:
        directory = tempfile.TemporaryDirectory()
        self.addCleanup(directory.cleanup)
        root = Path(directory.name)
        provider_regions = [
        ...
        payload = json.loads(output.getvalue())
        self.assertEqual(code, 0)
        self.assertFalse(payload["summary"]["replication_enabled"])
        self.assertEqual(payload["summary"]["replication_target_regions"], [])
        self.assertEqual(payload["summary"]["replication_target_source"], "replication_disabled")
        self.assertEqual(payload["replication_plan"]["status"], "skipped")
        self.assertEqual(payload["region_policy"]["path"], "policy/region-policy.toml")
    ...
    def test_config_validate_reports_capture_replicate_policy_inputs(self) -> None:
        ...
        self.assertTrue(payload["effective_defaults"]["replication_enabled"])
        ...
        self.assertIn(
            {"field": "replication_enabled", "source": "[capture-replicate-deploy].replication_enabled"},
            payload["sources"],
        )
